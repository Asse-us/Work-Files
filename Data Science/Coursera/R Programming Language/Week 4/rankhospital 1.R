...
rankhospital1 <- function(state, outcome, num = "best") {
        ...
        tempData  <- final.Data[order(as.numeric(final.Data[[Column.Name]]),final.Data[["Hospital.Name"]],decreasing=FALSE,na.last=NA), ]                
        ...
        # Finding all hospitals with this mortaility rate
        # all.hospitals.with.current.rank  <- final.Data[final.Data[Column.Name] == as.numeric(mortaility.for.given.rank),"Hospital.Name"]
        #all.hospitals.with.current.rank  <- tempData[tempData[Column.Name] == as.numeric(mortaility.for.given.rank),"Hospital.Name"]
        
        
        #sort(all.hospitals.with.current.rank)[1]
        tempData[rank,"Hospital.Name"]
}
